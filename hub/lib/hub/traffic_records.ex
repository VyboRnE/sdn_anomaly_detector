  ...
  query =
    from(sq in subquery(base_query),
      ...
      select: %{
        ...
        cusum_anomaly_count: fragment("SUM(CASE WHEN ? = TRUE THEN 1 ELSE 0 END)", sq.cusum_anomaly),
        isolation_anomaly_count: fragment("SUM(CASE WHEN ? = TRUE THEN 1 ELSE 0 END)", sq.isolation_anomaly),
        proto_tcp_count:
          fragment("SUM(COALESCE((? ->> '6')::int, 0))", sq.proto_counter),
        proto_udp_count:
          fragment("SUM(COALESCE((? ->> '17')::int, 0))", sq.proto_counter),
        proto_icmp_count:
          fragment("SUM(COALESCE((? ->> '1')::int, 0))", sq.proto_counter)
      }
    )
